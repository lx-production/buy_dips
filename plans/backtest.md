...
- Hoàn thiện offline backtest `support_close_v2`, BUY CSV và một chart server riêng.
...
- Replay engine trong `src/trading/backtest.py`:
  ...
  - Cold cache miss ingest 4h history một lần vào `IncrementalZoneDetectorState`, rồi `snapshot_evidence` + full materialization tại từng watermark thiếu. Cache lưu candidate pre-track; replay luôn chạy `ZoneTrackState` theo thứ tự watermark (bootstrap snapshot đầu). Fingerprint cooldown là `zone_track_id`. Warm path (mọi snapshot đều hit) không khởi tạo incremental evidence state, nhưng vẫn advance tracks.
...
- Hover nến hiện OHLC; hover BUY / zone append chi tiết. Zone tooltip gồm `origin`, `bounds_style`, `score`, `revision_fingerprint`, track/lineage ids. Không có marker/bảng/tooltip dành cho no-BUY.
