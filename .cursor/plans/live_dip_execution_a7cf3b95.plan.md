...
overview: Add a fail-closed Polygon trading path that keeps the existing Binance 4H zone detector unchanged, fetches only Binance BTCUSDT 1h candles into the existing candles table, derives closed 4H bars from those 1h candles to rebuild zones, and buys exactly 1 USDT of PRANA through the in-house swap quote API (POST /api/swap/quote on the local route server). Retire the Phase 1 paper scorer and signals table in favor of one support_close_v1 decision engine and one decisions schema shared by observe, dry_run, and live. Buy on two paths: (1) a closed 1h candle closes inside a support zone strictly below the zone midpoint, with the 48h pre-entry high strictly above the midpoint of the internal range to the next higher zone; or (2) a closed 1h candle closes below that support, within the 70%–100% portion of the space measured upward from the next-lower zone's high to the broken zone's low, after the 48h lookback confirms that price dropped below the broken zone's lower bound, and no BUY exists in the prior 24h lookback. Use separate dev/prod keystores; Pi prod runs 24/7 via systemd with LoadCredential for secrets.
todos:
  - id: data-signal
    content: Retire paper scorer/signals table; add support_close_v1 engine (Path A inside-below-mid + Path B below-zone 70–100% band / 24h no-buy) + decisions schema; fetch-only-1h; derive closed 4H; rebuild zones on watermark.
    status: pending
  - id: wallet-safety
  ...
  - id: audit-risk
    ...
    status: pending
  - id: tests
    content: Replace paper-signal tests with support-close/internal-range gates plus below-zone 70–100% band and 24h no-buy gates; cover 1h fetch, 1h-to-4h, zone watermark, risk, storage, in-house quote response validation, runner, and tx lifecycle.
    status: pending
  - id: backtest
...
- Execute a market swap shortly after the UTC hour through the **in-house swap quote API** on Polygon (chain ID 137): `POST http://127.0.0.1:4173/api/swap/quote` with `tokenInSymbol=USDT`, `tokenOutSymbol=PRANA`, `amountIn="1"`. Routing, calldata, and router selection come from this local server only.
...
- Output is gate-based, not scored. Every persisted decision has exactly one `decision` (`BUY` or `HOLD`) and exactly one `reason_code` from the closed set below. Two entry paths can produce `BUY`; evaluate path selection first, then that path’s gates. The first matching failure wins:
  - **Path A — inside support (below mid):** `zone.low <= close < zone.mid`, plus the existing higher-zone / 48h pre-entry-high gates.
  - **Path B — close entirely below support:** `close < zone.low`, only when the 48h lookback shows price dropped under that zone’s lower bound into the **70%–100% band** of the gap `(next_lower_zone.high → zone.low)`, and there is **no prior `BUY` in the 24h lookback**.
- Reason codes (closed set):
  1. `CLOSE_OUTSIDE_SUPPORT` → `HOLD`: close is above every support (`close > zone.high` for all), or below the lowest zone with no usable next-lower gap, or otherwise not on Path A or Path B.
  2. `CLOSE_NOT_BELOW_ZONE_MID` → `HOLD`: Path A only — close is inside the selected zone but at or above its midpoint; Path A requires `zone.low <= close < zone.mid`.
  3. `NO_HIGHER_ZONE` → `HOLD`: Path A only — the selected support has no usable higher support above it (none exists, or the nearest above touches/overlaps so no positive internal range).
  4. `HIGH_NOT_ABOVE_INTERNAL_MID` → `HOLD`: Path A only — no pre-entry high strictly above `internal_range_midpoint` (empty 48h/pre-entry set, or `pre_entry_closed_high <= internal_range_midpoint`).
  5. `NO_LOWER_ZONE` → `HOLD`: Path B only — selected support has no usable next-lower support below it (`next_lower.high < zone.low` with a positive gap), so the below-zone band cannot be formed.
  6. `BELOW_ZONE_OUT_OF_BAND` → `HOLD`: Path B only — `close < zone.low` but not inside the 70%–100% band of `(next_lower.high → zone.low)` (see algorithm), or the 48h lookback never printed a low under `zone.low` into that support.
  7. `RECENT_BUY_IN_24H` → `HOLD`: Path B only — a persisted `BUY` decision already exists with trigger `open_time` in `[trigger.open_time - 24h, trigger.open_time)`.
  8. `BUY_GATES_PASSED` → `BUY`: Path A or Path B gates all passed.
...
- Shared payload for every cycle: current closed 1h candle/time, selected zone fingerprint and bounds (including zone mid), path (`inside_below_mid` / `below_zone_band`), adjacent higher-zone and/or next-lower-zone fingerprints and bounds as needed, internal-range midpoint (Path A), below-zone band bounds + close position pct (Path B), 48h lookback window, pre-entry closed-candle high (Path A), 24h prior-BUY flag (Path B), gate results, whether zones rebuilt this cycle, and strategy/config version.
...
4. Sort active support zones from low to high. For each candidate `zone`:
   - **Path A pair (higher):** nearest distinct `higher_zone` where `higher.low > zone.high`, and define:
     ...
     If there is no such higher zone (none above, or every neighbor above touches/overlaps), Path A for that candidate gets `NO_HIGHER_ZONE`.
   - **Path B pair (lower):** nearest distinct `next_lower_zone` where `next_lower.high < zone.low`, and define the below-zone gap:
     - `below_gap_low = next_lower_zone.high`;
     - `below_gap_high = zone.low`;
     - position pct with **0% = `below_gap_low`**, **100% = `below_gap_high`**:
       `below_zone_pct = (close - below_gap_low) / (below_gap_high - below_gap_low)`;
     - Path B band = **70% ≤ `below_zone_pct` ≤ 100%** and `close < zone.low` (so eligible closes sit in the upper part of the gap, nearer the broken support than the next-lower zone). Practically `0.70 <= below_zone_pct < 1.0` because a close exactly at `zone.low` is Path A territory / on the bound, not “entirely below”.
     If there is no such next-lower zone, Path B for that candidate gets `NO_LOWER_ZONE`.
5. Define the completed pre-entry leg without adding new candle state, capped to a **48h lookback** (closed 1h candles in DB with `open_time` in `[trigger.open_time - 48h, trigger.open_time)`):
   - also ignore candles older than `zone_source_time` when that is newer than the 48h floor;
   - find the most recent earlier candle in this window whose range intersects the candidate zone (the previous support touch/bounce); evaluate only candles strictly after that candle;
   - if no earlier touch exists, evaluate all closed candles in the window before the trigger;
   - `pre_entry_closed_high` is the maximum `high` from those candles, excluding both the earlier touch candle and the current trigger candle. An empty set fails Path A’s high gate as `HIGH_NOT_ABOVE_INTERNAL_MID`. Do not require perfect contiguity of every hourly slot.
   - For Path B, the same 48h window must show that price **dropped under this support**: at least one closed candle in the window (including the trigger) has `low < zone.low` (broke the lower bound). If that never happened, Path B fails as `BELOW_ZONE_OUT_OF_BAND`.
6. Select a zone / path:
   - **Path A:** current closed 1h `close` is inside the support and **strictly below** the zone midpoint (`zone.low <= close < zone.mid`); closes at/above `zone.mid` up through `zone.high` are `HOLD` (`CLOSE_NOT_BELOW_ZONE_MID` when inside); and `pre_entry_closed_high` is **strictly greater than** `internal_range_midpoint` (exactly 50% is `HOLD`).
   - **Path B (additional):** current closed 1h `close` is **entirely below** the support (`close < zone.low`), `below_zone_pct` is in the **70%–100%** band of `(next_lower.high → zone.low)` as defined above, the 48h lookback recorded a break under `zone.low`, and there is **no persisted `BUY`** whose trigger `open_time` falls in `[trigger.open_time - 24h, trigger.open_time)`. A recent buy fails Path B as `RECENT_BUY_IN_24H` (scan `decisions` only—no separate lock table). Path B does **not** require the Path A internal-range high gate.
   - Prefer Path A when the close is inside a zone. When close is below all containing zones, evaluate Path B against the nearest support above the close (the broken zone whose `low` is just above `close`, i.e. highest `zone.low` still `> close` among zones with `close < zone.low`). If more than one zone contains the close for Path A, choose the one with the highest `low`.
...
- Add `src/trading/` with focused modules:
  ...
  - `signal.py` and `zone_identity.py`: the sole support-close decision engine (Path A inside-below-mid + Path B below-zone 70–100% band) and stable source-time fingerprint. Keep the 48h backward scan, midpoint calculation, below-zone pct helper, and 24h prior-BUY lookup here as small pure helpers; replace [src/signals.py](src/signals.py) and do not leave a parallel scorer.
  ...
  - `prana_swap.py`: a thin HTTP adapter for `POST /api/swap/quote` plus strict response/transaction/`deadline` validation. Never send an `Origin` header. Do not add protocol-specific route selection; the local server owns routing.
...
- Default execution checks: Path A (1h close inside support and strictly below zone mid + 48h pre-entry lookback + strict internal-range `> 50%` pre-entry-high) or Path B (1h close entirely below support in the 70%–100% band of `zone.low → next_lower.high`, 48h under-bound drop, no BUY in 24h lookback), 0.50% slippage (`slippageBps=50`), quote must finish before `deadline` (~3 min), configurable maximum gas, minimum POL reserve, a short maximum quote-response age, and a `data/PAUSE_TRADING` kill switch. No Binance-to-DEX PRANA price-deviation gate.
...
- Replace [tests/test_signals.py](tests/test_signals.py) and any paper-score assertions with one direct test for every reason code plus precedence tests proving only the first failed gate is stored. Cover Path A: empty zones / close outside support; close at zone low; close just below/at/above zone mid; no higher zone including overlapping/touching neighbor; empty pre-entry leg and high below/equal/above internal-range midpoint all under the high gate; 48h cap ignoring older highs/touches; first approach; prior bounce inside window; weak retest after a shallow bounce; trigger-candle high excluded. Cover Path B: close entirely below zone inside/outside the 70%–100% band of `(next_lower.high → zone.low)`; no next-lower zone; 48h window with/without a low under `zone.low`; 24h lookback with/without a prior `BUY`; Path A preferred when close is inside a zone. Also cover: open candle ignored in favor of latest closed DB row; `BUY_GATES_PASSED` for both paths; closed 1h upsert/read; 1h→4h aggregation and incomplete-bucket rejection; rebuild-zones-only-on-new-4h watermark; zone identity; token units; minimum-output rounding; config/risk gates; SQLite decision idempotency; execution skip reasons preserving the original `BUY`; encrypted-keystore handling in a temporary directory; redaction; runner retries; pending/confirmed/reverted reconciliation; and a small synthetic backtest replay.
