...
```text
zone_rebuild_watermark:binance:BTCUSDT:4h:support_structure_v1
```
...
- Bootstrap when the key is absent:
  1. Require at least one usable latest completed closed 4H candle and enough history for `support_structure_v1`; otherwise abort with no decision.
