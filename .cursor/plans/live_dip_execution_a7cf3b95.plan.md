...
- Reason codes (closed set):
  1. `CLOSE_OUTSIDE_ENTRY_REGION` → `HOLD`: the current close is neither inside the selected support below the 70% level nor immediately below it in the 50%–100% band.
  2. `CLOSE_NOT_BELOW_ZONE_MID` → `HOLD`: the close is inside the selected support but at or above 70% of the zone span (`0%` = `zone.low`, `100%` = `zone.high`); the inside-zone entry region requires `zone.low <= close` and `inside_pct < 0.70`.
