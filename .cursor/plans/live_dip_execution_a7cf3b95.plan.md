...
- Output is gate-based, not scored. Every persisted decision has exactly one `decision` (`BUY` or `HOLD`) and exactly one `reason_code` from the closed set below. Two entry paths can produce `BUY`; evaluate path selection first, then that path’s gates. The first matching failure wins:
  ...
  - **Path B — close entirely below support:** `close < zone.low` in the **70%–100% band** of the gap `(next_lower_zone.high → zone.low)`, plus the **same** higher-zone / 48h pre-entry-high gate as Path A (`pre_entry_closed_high > internal_range_midpoint`), and **no prior `BUY` in the 24h lookback**.
- Reason codes (closed set):
  ...
  3. `NO_HIGHER_ZONE` → `HOLD`: Path A or Path B — the selected support has no usable higher support above it (none exists, or the nearest above touches/overlaps so no positive internal range); both paths need `internal_range_midpoint` for the 48h pre-entry-high gate.
  4. `HIGH_NOT_ABOVE_INTERNAL_MID` → `HOLD`: Path A or Path B — no pre-entry high strictly above `internal_range_midpoint` (empty 48h/pre-entry set, or `pre_entry_closed_high <= internal_range_midpoint`).
  ...
  6. `BELOW_ZONE_OUT_OF_BAND` → `HOLD`: Path B only — `close < zone.low` but not inside the 70%–100% band of `(next_lower.high → zone.low)` (see algorithm).
...
- Shared payload for every cycle: current closed 1h candle/time, selected zone fingerprint and bounds (including zone mid), path (`inside_below_mid` / `below_zone_band`), adjacent higher-zone and/or next-lower-zone fingerprints and bounds as needed, internal-range midpoint (both paths), below-zone band bounds + close position pct (Path B), 48h lookback window, pre-entry closed-candle high (both paths), 24h prior-BUY flag (Path B), gate results, whether zones rebuilt this cycle, and strategy/config version.
...
4. Sort active support zones from low to high. For each candidate `zone`:
   - **Path A pair (higher):** nearest distinct `higher_zone` where `higher.low > zone.high`, and define:
     ...
     If there is no such higher zone (none above, or every neighbor above touches/overlaps), Path A or Path B for that candidate gets `NO_HIGHER_ZONE` (both need the midpoint for the shared 48h high gate).
...
5. Define the completed pre-entry leg without adding new candle state, capped to a **48h lookback** (closed 1h candles in DB with `open_time` in `[trigger.open_time - 48h, trigger.open_time)`). **Shared by Path A and Path B**—same window, same `pre_entry_closed_high`, same comparison to `internal_range_midpoint`:
   ...
   - `pre_entry_closed_high` is the maximum `high` from those candles, excluding both the earlier touch candle and the current trigger candle. An empty set (or `pre_entry_closed_high <= internal_range_midpoint`) fails as `HIGH_NOT_ABOVE_INTERNAL_MID` on **either** path. Do not require perfect contiguity of every hourly slot.
   - There is **no** separate Path B “48h drop under `zone.low`” check—that was wrong (`close < zone.low` already implies the trigger’s `low` broke the bound).
6. Select a zone / path:
   ...
   - **Path B (additional):** current closed 1h `close` is **entirely below** the support (`close < zone.low`), `below_zone_pct` is in the **70%–100%** band of `(next_lower.high → zone.low)` as defined above, `pre_entry_closed_high` is **strictly greater than** `internal_range_midpoint` (same gate as Path A), and there is **no persisted `BUY`** whose trigger `open_time` falls in `[trigger.open_time - 24h, trigger.open_time)`. A recent buy fails Path B as `RECENT_BUY_IN_24H` (scan `decisions` only—no separate lock table).
   - Prefer Path A when the close is inside a zone (zone finder yields at most one zone containing a given close). For Path B, only consider the **nearest support above the close** (price just broke that zone’s lower bound)—the zone with the smallest `low` still above `close`:
     `selected = min((z for z in zones if z.low > close), key=lambda z: z.low)`.
     Example: zones with `low` at 60k / 70k / 80k and `close` 65k → Path B uses the 70k zone (not 80k).
