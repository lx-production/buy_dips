...
todos:
  ...
  - id: tests
    ...
    status: pending
  - id: backtest
    content: Offline support_close_v1 replay in src/trading/backtest.py with isolated in-memory prior-BUY state; CLI backtest --start/--end/--csv (BUY-only summary+CSV); shared build_fingerprinted_support_zones helper; serve_backtest_chart.py with GET /api/backtest (no HOLD).
    status: completed
  - id: docs-rollout
...
- Keep `support_structure_v1` in [src/zones/](src/zones/) as the swing detector, plus a last-step **persistent wick-floor** overlay: a closed 4H local swing low whose wick hangs at least `$500` below the body pins `low = wick`, `high = wick + 500` (`origin=persistent_wick_floor`, one touch, frozen source candle). Later deeper lows, macro-merge, and daily overlay must not move or drop that shelf; overlapping persistent floors keep the oldest. Never feed an open/incomplete 4H bar into the detector (no look-ahead / fake pivots).
...
- Output is gate-based, not scored. Every persisted decision has exactly one `decision` (`BUY` or `HOLD`) and exactly one `reason_code` from the closed set below. There is one **dip-to-support** entry flow. The trigger closed 1h candle must be **red** (`close < open`); a green or doji candle never selects a zone. After that, the current close may qualify in either of two adjacent entry regions:
  - **Inside support:** `zone.low <= close` and `inside_pct < 0.80`, where `inside_pct = (close - zone.low) / (zone.high - zone.low)` so `0%` = `zone.low` and `100%` = `zone.high`.
...
- Reason codes (closed set):
  ...
  2. `CLOSE_OUTSIDE_ENTRY_REGION` → `HOLD`: the current close is neither inside the selected support below the 80% level nor immediately below it in the 50%–100% band.
  3. `CLOSE_NOT_BELOW_ZONE_MID` → `HOLD`: the close is inside the selected support but at or above 80% of the zone span (`0%` = `zone.low`, `100%` = `zone.high`); the inside-zone entry region requires `zone.low <= close` and `inside_pct < 0.80`.
  4. `NO_HIGHER_ZONE` → `HOLD`: the selected support has no usable higher support above it (none exists, or the nearest above touches/overlaps), so a positive internal range and its midpoint cannot be formed.
  5. `NO_RECENT_CLOSE_ABOVE_INTERNAL_MID` → `HOLD`: no earlier closed 1h candle in the effective 48h lookback has `close > internal_range_midpoint`.
  6. `NO_LOWER_ZONE` → `HOLD`: the current close is below the selected support, but there is no usable next-lower support (`next_lower.high < zone.low`) from which to form the below-zone band.
  7. `BELOW_ZONE_OUT_OF_BAND` → `HOLD`: the current close is below `zone.low` but not inside the 50%–100% band of `(next_lower.high → zone.low)`.
  8. `RECENT_BUY_IN_24H` → `HOLD`: a persisted `BUY` decision already exists for the **same selected zone fingerprint** with trigger `open_time` in `[trigger.open_time - 24h, trigger.open_time)`. A prior BUY at a different zone (e.g. shallower A while now evaluating deeper B) does not trigger this reason.
  9. `BUY_GATES_PASSED` → `BUY`: all unified dip-to-support gates passed.
...
- Shared payload for every cycle: current closed 1h candle/time, `zone_set_as_of`, fingerprint version, selected zone fingerprint and bounds (including zone mid), selected `zone_source_time` / `source_open_times`, entry region (`inside_below_80` / `below_zone_band`), adjacent higher-zone and optional next-lower-zone fingerprints and bounds, internal-range midpoint, below-zone band bounds + close position pct when below the zone, effective 48h lookback bounds (including any `zone_source_time` floor), selected dip-origin candle time/close, per-zone 24h prior-BUY flag (same fingerprint only), gate results, whether zones rebuilt this cycle, and strategy/config version.
...
4. Use `zones = detector_result["support"]` (already sorted low→high in the detector; `active` is always empty). Do **not** filter by `price_state == "support"`: a zone currently above price is classified `resistance`/`active` by the detector, but Path B (immediately-below-zone entry) still needs that broken/overhead support in the candidate list. Before selecting a zone, require the trigger 1h candle to be **red** (`close < open`); if `close >= open`, return `CLOSE_NOT_BELOW_OPEN` and do not select a zone. Then select the nearest support reached by the current close, and validate the current entry region before evaluating dip history:
   - if the close is inside one or more zones (`zone.low <= close <= zone.high`):
     ...
     - require `selected.low <= close` and `inside_pct < 0.80`, where `inside_pct = (close - selected.low) / (selected.high - selected.low)` (`0%` = `selected.low`, `100%` = `selected.high`);
     - if `inside_pct >= 0.80`, return `CLOSE_NOT_BELOW_ZONE_MID`;
   - otherwise:
...
7. Buy only when all unified entry conditions pass:
   ...
   - the current closed 1h `close` is either **inside the selected support and strictly below 80% of the zone span** (`zone.low <= close` and `inside_pct < 0.80`, where `0%` = `zone.low` and `100%` = `zone.high`) or **immediately below the support in the 50%–100% band** (`close < zone.low` and `0.50 <= below_zone_pct <= 1.0`; the separate `close < zone.low` condition excludes the 100% boundary);
...
- Add `src/trading/` with focused modules:
  ...
  - `signal.py`: the sole unified dip-to-support decision engine (red trigger candle, then inside-below-80 or below-zone 50–100% entry region). Reads already-persisted fingerprints / `zone_source_time`; keep nearest-support selection, the 48h backward scan for the nearest qualifying close, midpoint calculation, below-zone pct helper, and per-zone 24h prior-BUY lookup (same fingerprint) here as small pure helpers; replace [src/signals.py](src/signals.py) and do not leave a parallel scorer. Pure engine also accepts `mode="backtest"` (not a `decisions.mode` schema value).
  ...
  - `backtest.py`: offline replay that walks historical closed 1h candles, rebuilds zones only on newly completed closed 4H bars via the shared fingerprinted-zone helper, and calls the same `support_close_v1` engine with `mode="backtest"`. Keep a **per-replay isolated prior-BUY list** (in-memory) for the per-zone 24h cooldown—do **not** read `decisions` from `observe`/`dry_run`/`live` or from a previous backtest run. Same input must yield the same BUY timestamps. CLI prints a BUY-only summary and writes the locked BUY CSV. Optional visual chart is served by [src/backtest_chart_server.py](src/backtest_chart_server.py) / [scripts/serve_backtest_chart.py](scripts/serve_backtest_chart.py) (`GET /api/backtest` returns meta/candles/buys/zone_segments only—never HOLD). No swap quote, wallet, or gas simulation in this phase.
...
- Method ([src/trading/backtest.py](src/trading/backtest.py)): walk each closed 1h candle in `[start, end)`; rebuild zones only when a newer closed 4H bar appears via shared `build_fingerprinted_support_zones`; call `evaluate_support_close_v1(..., mode="backtest")`; assume fill at the trigger candle `close` for summary only.
...
- **Isolated cooldown state:** each backtest run keeps its own in-memory prior-BUY list (`trigger_open_time` + zone fingerprint). Do not mix with live `decisions`. Re-running the same window emits the same BUY timestamps.
- Output:
