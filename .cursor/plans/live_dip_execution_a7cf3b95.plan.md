...
todos:
  ...
  - id: tests
    ...
    status: pending
  - id: backtest
    content: Extend 1h multi-month backfill (Phase 1 candles backfill is 4h-only); add offline support_close_v1 replay with isolated prior-BUY state; print BUY/HOLD summary and full BUY CSV (trigger/entry/zone/dip/zone_set_as_of columns) before observe/dry_run/live.
    status: pending
  - id: docs-rollout
    content: Update README/study to drop paper-score docs; document decisions schema, 1h backfill + backtest BUY CSV gate, dev/prod keystores and quote hosts (prod loopback vs https://prana.triethocduongpho.net), Pi systemd LoadCredential units, in-house USDT→PRANA swap flow, and observe/dry-run/capped-live rollout.
    status: pending
isProject: false
---
...
- Both regions share the same setup gates: a nearest qualifying dip-origin candle exists in the 48h lookback with `dip_origin.close > internal_range_midpoint`, and no prior `BUY` for the **same selected zone fingerprint** exists in the prior 24h. Cooldown is **per-zone**, not global: a BUY at shallower zone A does not block a later BUY at deeper zone B within 24h. The prior-BUY source is mode-specific: `observe`/`dry_run`/`live` read persisted `decisions`; backtest reads only that replay's isolated prior-BUY list.
- Reason codes (closed set):
  ...
  7. `RECENT_BUY_IN_24H` → `HOLD`: a persisted `BUY` decision already exists for the **same selected zone fingerprint** with trigger `open_time` in `[trigger.open_time - 24h, trigger.open_time)`. A prior BUY at a different zone (e.g. shallower A while now evaluating deeper B) does not trigger this reason.
...
- Shared payload for every cycle: current closed 1h candle/time, selected zone fingerprint and bounds (including zone mid), selected `zone_source_time` / `source_open_times`, entry region (`inside_below_mid` / `below_zone_band`), adjacent higher-zone and optional next-lower-zone fingerprints and bounds, internal-range midpoint, below-zone band bounds + close position pct when below the zone, effective 48h lookback bounds (including any `zone_source_time` floor), selected dip-origin candle time/close, per-zone 24h prior-BUY flag (same fingerprint only), gate results, whether zones rebuilt this cycle, and strategy/config version.
...
3. From closed 1h candles, decide how to treat the Binance-aligned 4H bucket relative to the zone watermark:
   ...
   - if newer, run `support_structure_v1` on closed 4h candles only; then in `zone_refresh` (not inside the detector, not in the signal engine) resolve each zone's detector `source_indexes` into stable `source_open_times` **before** persistence and fingerprinting; advance the watermark;
   ...
   - for each zone: `source_open_times = [source_df.iloc[i]["open_time"] for i in source_indexes]`; `zone_source_time = max(source_open_times)`; build the stable zone fingerprint from those open times (plus bounds/origin as needed); persist `source_open_times`, `zone_source_time`, and fingerprint with the zone set;
...
7. Buy only when all unified entry conditions pass:
   ...
   - there is **no prior `BUY` for the same selected zone fingerprint** whose trigger `open_time` falls in `[trigger.open_time - 24h, trigger.open_time)`. Match on the selected zone fingerprint stored with that prior BUY (not global across all zones). A recent same-zone buy returns `RECENT_BUY_IN_24H`; a BUY at a different zone within 24h does not. Live/`observe`/`dry_run` scan the `decisions` table only—no separate lock table. Backtest uses an isolated prior-BUY store for the current replay (see Offline backtest)—never the live `decisions` table. Intentional DCA: if price continues down to a deeper support within 24h, that deeper zone may still BUY.
...
- Add `src/trading/` with focused modules:
  ...
  - `zone_refresh.py`: compare derived/latest closed 4h `open_time` against the rebuild watermark; rebuild zones only when newer; immediately resolve `source_indexes` → `source_open_times` / `zone_source_time` (4H frame vs daily frame for `source_timeframe="1d"`); persist those times with the zone set; expose `detector_result["support"]` (full support list, not `active` / not `price_state`-filtered) for the signal.
  - `zone_identity.py`: pure helpers to resolve source indexes against the correct dataframe, compute `zone_source_time = max(source_open_times)`, and build the stable fingerprint. Called from `zone_refresh` at rebuild time only—not from the detector and not from the hourly signal path.
  - `signal.py`: the sole unified dip-to-support decision engine (inside-below-mid or below-zone 70–100% entry region). Reads already-persisted fingerprints / `zone_source_time`; keep nearest-support selection, the 48h backward scan for the nearest qualifying close, midpoint calculation, below-zone pct helper, and per-zone 24h prior-BUY lookup (same fingerprint) here as small pure helpers; replace [src/signals.py](src/signals.py) and do not leave a parallel scorer.
  ...
  - `backtest.py`: offline replay that walks historical closed 1h candles, rebuilds zones only on newly completed closed 4H bars, and calls the same `support_close_v1` engine. Keep a **per-replay isolated prior-BUY list** (in-memory preferred; optional `backtest_run_id` only if persistence is needed later) for the per-zone 24h cooldown—do **not** read `decisions` from `observe`/`dry_run`/`live` or from a previous backtest run. Same input must yield the same BUY timestamps. Print CLI summary and write a full BUY CSV with the locked columns (`trigger_time`, `trigger_close`, `entry_region`, zone bounds/mid, `higher_zone_low`, `internal_range_midpoint`, `next_lower_zone_high`, `below_zone_pct`, dip-origin time/close, `zone_set_as_of`). No swap quote, wallet, or gas simulation in this phase.
...
- Decision columns (conceptual): candle open/close times and reference close, entry region, selected/higher/optional next-lower zone fingerprints and bounds, selected `zone_source_time` / `source_open_times`, internal-range midpoint, optional below-zone band bounds + pct, effective 48h window bounds, dip-origin candle time/close, per-zone 24h prior-BUY flag (same fingerprint), `decision`, the closed-set `reason_code`, gate JSON, zones-rebuilt flag, mode, strategy/config version, sanitized error. Execution columns: decision id, execution status/skip/failure reason, quote server/router version summary, sanitized route summary, quote/`minimumAmountOut`, nonce, transaction hash, receipt fields, actual PRANA output, status. Never persist API keys, raw calldata, signed transactions, or `verification.token`.
- Persisted zone rows (or the zone-set snapshot used between rebuilds) must store resolved `source_open_times`, `zone_source_time`, and fingerprint alongside the existing bounds/`source_indexes`. Indexes alone are not enough for later hourly cycles or cooldown matching.
...
- Goal: prove `support_close_v1` fires sensibly on history before spending canary capital—especially **where** each BUY landed. This is signal-only replay, not a full PnL/portfolio simulator.
- **1h historical backfill (required):** Phase 1 [src/candles.py](src/candles.py) `backfill_12_months` only supports `timeframe="4h"` (`INTERVAL_MS` has no `1h`). Extend backfill to `1h` (multi-month window) so backtest has enough closed hourly candles; then derive closed 4H from those 1h rows (or keep existing 4h rows in sync). Do not invent a second candle store. Document / expose a CLI path (e.g. extend existing backfill command or `trade`/`backtest` prep) that fetches closed Binance `BTCUSDT` `1h` history into `candles`.
- Input: already stored closed Binance `BTCUSDT` `1h` candles (and derived/closed `4h` bars). Reuse the extended 1h backfill + 1h→4h aggregation.
...
- **Isolated cooldown state (required):** the per-zone 24h gate must not scan the live `decisions` table. Each backtest run keeps its own prior-BUY list for that replay only (in-memory list of `{trigger_open_time, zone_fingerprint}` appended when the engine returns `BUY`; or a dedicated `backtest_run_id` scoped store if rows are persisted). Do not mix with `observe`/`dry_run`/`live` BUYs or leftover rows from an earlier backtest. Re-running the same candle window must emit the exact same BUY timestamps.
- Output (this phase, not optional later):
  - Compact CLI summary — candle count, zone rebuild count, `BUY` count, `HOLD` reason-code tallies.
  - **Export every BUY row to CSV** (all BUYs, not a short list) with columns:
    - `trigger_time`
    - `trigger_close`
    - `entry_region`
    - `zone_low`
    ...
    - `dip_origin_close`
    - `zone_set_as_of`
  - Each CSV row must let the operator confirm: dip origin (from where price came down), close above internal midpoint, inside-support vs below-support entry, below-zone 70%–100% band when applicable, and which zone snapshot was used (`zone_set_as_of`). Do not build charts or a research framework in this phase.
...
- Update [README.md](README.md) and [study.md](study.md): remove Phase 1 paper-score / `signals` table docs; describe the unified dip-to-support algorithm (nearest qualifying 1h close above the internal midpoint within 48h, current close inside-below-mid or below-zone 70%–100%, and per-zone 24h no-buy so a deeper zone may still BUY within 24h), offline backtest gate (1h multi-month backfill + full BUY CSV columns), `decisions`/`trade_executions` schema, dual-timeframe flow (fetch 1h only; derive closed 4H; rebuild on watermark), in-house `POST /api/swap/quote` USDT→PRANA flow with **prod** `http://127.0.0.1:4173` vs **dev** `https://prana.triethocduongpho.net` (approve → broadcast before deadline), modes, dev/prod keystore separation, Pi systemd `LoadCredential` setup for keystore password, direct capped router approval/revocation, audit queries, pause/recovery, and buy-only scope. Note that recreating the local DB drops old paper signal rows.
