...
- Reason codes (closed set):
  1. `CLOSE_OUTSIDE_ENTRY_REGION` → `HOLD`: the current close is neither inside the selected support strictly below its midpoint nor immediately below it in the 50%–100% band.
...
7. Buy only when all unified entry conditions pass:
   - the current closed 1h `close` is either **inside the selected support and strictly below its midpoint** (`zone.low <= close < zone.mid`) or **immediately below the support in the 50%–100% band** (`close < zone.low` and `0.50 <= below_zone_pct <= 1.0`; the separate `close < zone.low` condition excludes the 100% boundary);
