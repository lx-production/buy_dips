...
- Tạm trao ownership repo cho `botuser`; chỉ chạy pip khi `requirements.txt` thực sự thay đổi; chạy pytest và skip `test_incremental_advance_matches_stateless_oracle_on_golden_prefixes` (test parity incremental chậm nhất, đã cover trên dev/CI).
...
- [ ] Pytest pass trên Pi (updater skip `test_incremental_advance_matches_stateless_oracle_on_golden_prefixes`).
