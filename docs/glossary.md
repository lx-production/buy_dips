...
**Structured audit log** — JSON Lines ra stdout và rotating file. `timestamp` là UTC+7 (`YYYY-MM-DD HH:MM:SS +07:00`). Event `decision_persisted` log `reference_close` (close của nến 1h vừa đóng) cùng low/high của selected, higher, và next-lower zone (không log fingerprint). Có correlation ID và trạng thái; password, key, RPC URL, calldata, signed bytes và verification token bị redacted.
